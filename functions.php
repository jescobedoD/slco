	...
	function salco_setup(){
		...
		if(isset($_POST['cms_salco'])){
			...
			switch($opcion){
				...
				case "consulta":
				case "registro":
				case "registro_campanas":
				case "modificar":
				case "modificar_campanas":
				case "avatar":
	...
	function salco_login(){
		...
		if(is_wp_error($user)){
			...
			if(isset($user->errors["incorrect_password"])){
				$wp_error->add('login_usuario', __('El nombre de usuario y/o contrase&ntilde;a ingresada no son v&aacute;lidos'));
				return;
			}
			if(isset($user->errors["invalid_username"])){
				...
				if($user_id){
                    ...
                    <script>_gaq.push(['_trackPageview', '/ingreso_sitio_privado/login']);</script>";
...
if(!function_exists('salco_contacto')){
	function salco_contacto(){
		...
		if ($asunto == "Exclusivo Tarjeta Crédito"){
			...
			//wp_mail("cacolinux@gmail.com", "Contacto", $mensaje, $headers);
		}
		 
		//wp_mail("candrade@sb.cl", "Contacto", $mensaje, $headers); # Camila Andrade
		//wp_mail("jespinosa@sb.cl", "Contacto", $mensaje, $headers); #Johanna Espinoza
		 wp_mail("egarate@sb.cl", "Contacto", $mensaje, $headers); # Evelyn Garate
		 wp_mail("bsotomayor@sb.cl", "Contacto", $mensaje, $headers); #Bernardita Sotomayor
	...
	function salco_registro(){
		...
		if(isset($_POST)){
			...
			if(isset($_POST['origen'])){
				$tel_particular = $_POST['tel_particular_pref'].$_POST['tel_particular_num'];
				$tel_celular = $_POST['tel_celular_pref'].$_POST['tel_celular_num'];

				if(($tel_particular != "") && (validar_cadena($tel_particular,'0123456789')!=true)){
	            	$wp_error->add('registro_usuario', __('El campo teléfono debe contener sólo números'));
				}
				
				if (($tel_celular != "") && (validar_cadena($tel_celular,'0123456789')!=true)){
					$wp_error->add('registro_usuario', __('El campo teléfono debe contener sólo números'));	
				}
			}else{
			...
			*/
			...
			if(!isset($_POST['origen'])){
				...
				if($_POST['r_acepto_politicas'] != 'true'){
					$wp_error->add('registro_usuario', __('Debes estar de acuerdo con las pol&iacute;ticas de privacidad'));
				}
			}

			
			if(isset($_POST['origen'])){
				$tel_fijo = mysql_real_escape_string($tel_particular);
				$tel_cel = mysql_real_escape_string($tel_celular);
			}else{
				$tel_fijo = mysql_real_escape_string($_POST['tel_particular']);
				$tel_cel = mysql_real_escape_string($_POST['tel_celular']);
			}


			
			$set = Array(
				'rut'				=> mysql_real_escape_string($_POST['rut_digito']),
				'DV'				=> mysql_real_escape_string($_POST['rut_verificador']),
				'primer_nombre'		=> mysql_real_escape_string($_POST['nombre']),
				'segundo_nombre'	=> mysql_real_escape_string($_POST['segundo_nombre']),
				...
				'fecha_nacimiento'	=> mysql_real_escape_string($_POST['fecha_de_nacimiento_dia']."-".$_POST['fecha_de_nacimiento_mes']."-".$_POST['fecha_de_nacimiento_anio']),
				'sexo'				=> mysql_real_escape_string($_POST['sexo']),
				'calle'				=> mysql_real_escape_string($_POST['dir_calle']),
				'user_numero'		=> mysql_real_escape_string($_POST['dir_numero']),
				'depto'				=> mysql_real_escape_string($_POST['dir_depto']),
				'comuna'			=> mysql_real_escape_string($_POST['comuna']),
				'provincia'			=> mysql_real_escape_string($_POST['provincia']),
				'region'			=> mysql_real_escape_string($_POST['region']),
				'telefono_fijo'		=> mysql_real_escape_string($tel_fijo),
				'telefono_celular'	=> mysql_real_escape_string($tel_cel),
			...
			if(email_exists($user_email) || username_exists($user_name)){
				...
				if($user_id){
					...
					if($_POST['origen'] == 'circulo'){
						$message = body_message_circulo("Registro Salcobrand", "Hola ".$set['primer_nombre']." ".$set['apellido_paterno']." ".$set['apellido_materno'], ",Bienvenido a Circulo Salcobrand");					
						$headers = "From: Salcobrand" . "\r\n";
						wp_mail($user_email, 'Bienvenido Salcobrand', $message, $headers);
					}else if($_POST['origen'] == 'campana'){
						$message = body_message_campana("Registro Salcobrand", "Hola ".$set['primer_nombre']." ".$set['apellido_paterno']." ".$set['apellido_materno'], ",Bienvenido a Salcobrand");					
						$headers = "From: Salcobrand" . "\r\n";
						wp_mail($user_email, 'Bienvenido a Salcobrand', $message, $headers);
					}
					else{
					...
					<script>_gaq.push(['_trackPageview', '/nuevo_registro']);</script>";
...
if(!function_exists('salco_registro_campanas')){
	function salco_registro_campanas(){
		global $wp_error;

		if(isset($_POST)){

			$set = Array(
				'rut'				=> reset(explode('-',mysql_real_escape_string($_POST['rut']))),
				'DV'				=> end(explode('-',mysql_real_escape_string($_POST['rut']))),
				'primer_nombre'		=> mysql_real_escape_string($_POST['nombre']),
				'segundo_nombre'	=> mysql_real_escape_string($_POST['segundo_nombre']),
				'apellido_paterno'	=> mysql_real_escape_string($_POST['apellido_paterno']),
				'apellido_materno'	=> mysql_real_escape_string($_POST['apellido_materno']),
				'fecha_nacimiento'	=> mysql_real_escape_string($_POST['dia']."-".$_POST['mes']."-".$_POST['ano']),
				'sexo'				=> mysql_real_escape_string($_POST['sexo']),
				'calle'				=> mysql_real_escape_string($_POST['dir_calle']),
				'user_numero'		=> mysql_real_escape_string($_POST['dir_numero']),
				'depto'				=> mysql_real_escape_string($_POST['dir_depto']),
				...
				'region'			=> mysql_real_escape_string($_POST['region']),
				'telefono_fijo'		=> mysql_real_escape_string($tel_fijo),
				'telefono_celular'	=> mysql_real_escape_string($_POST['celular']),
				'email'				=> mysql_real_escape_string($_POST['email']),
				'estado_civil'		=> mysql_real_escape_string($_POST['estado_civil']),
				'password'			=> mysql_real_escape_string($_POST['password']),
				'origen'			=> mysql_real_escape_string($_POST['origen'].' - '.$_SESSION['source']),
				'acepto'			=> (isset($_POST['acepto']) && $_POST['acepto']=="on")? "1" : "0",
				'acepto_sms'		=> (isset($_POST['acepto_sms']) && $_POST['acepto_sms']=="on")? "1" : "0",
			...
			if(email_exists($user_email) || username_exists($user_name)){
				...
				if($user_id){
					...
					update_user_meta($user_id, 'estado_civil', $set['estado_civil']);
					update_user_meta($user_id, 'avatar', '');
					update_user_meta($user_id, 'origen', $set['origen']);
					update_user_meta($user_id, 'acepto', $set['acepto']);
					update_user_meta($user_id, 'acepto_sms', $set['acepto_sms']);
					update_user_meta($user_id, 'modo_viaje', $_SESSION['modo']);
...
if(!function_exists('salco_modificar')){
	...
	function salco_modificar(){
		...
		<script>_gaq.push(['_trackPageview', '/ingreso_sitio_privado/actualizacion_datos']);</script>";
...
if(!function_exists('salco_modificar_campanas')){

	function salco_modificar_campanas(){
		...
		if(isset($_POST)){
			...
			if(!isset($_POST['apellido_paterno']) || $_POST['apellido_paterno']==""){
				$wp_error->add('registro_usuario', __('El campo apellido es obligatorio'));
			}
			...
			update_user_meta($user_id, 'apellido_paterno', $set['apellido_paterno'], $current_user->apellido_paterno);
			update_user_meta($user_id, 'telefono_celular', $set['telefono_celular'], $current_user->telefono_celular);
			update_user_meta($user_id, 'origen', $set['origen'], $current_user->origen);
			update_user_meta($user_id, 'acepto', $set['acepto']);
			update_user_meta($user_id, 'acepto_sms', $set['acepto_sms']);
			update_user_meta($user_id, 'modo_viaje', $_SESSION['modo']);
...
if(!function_exists('body_message_circulo')){
	function body_message_circulo($title="Salcobrand", $msj_title="", $msj_body=""){
		$html = '';
		$html .= '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">';
		$html .= '<html xmlns="http://www.w3.org/1999/xhtml">';
		$html .= '<head>';
		...
		$html .= '<title>'.$title.'</title>';
		$html .= '</head>';
		$html .= '<body style="margin:0 auto;">';
			$html .= '<div id="wrap" style="margin:0px;">';
			$html .= '<a href="http://www.salcobrand.cl/" target="blank_"><img src="http://www.salcobrand.cl/cl/wp-content/themes/salcobrand/images/emailing_bienvenida_circulo.jpg"></a>';
			$html .= '</div>';
		$html .= '</body>';
		$html .= '</html>';
		return $html;
	...
	function body_message_campana($title="Salcobrand", $msj_title="", $msj_body=""){
		$html = '<html>
		<head>
				...
				<table border="0" cellpadding="0" cellspacing="0" height="100%" width="100%" id="backgroundTable">
					<tr>
						<td align="center" valign="top">
							...
							<table border="0" cellpadding="0" cellspacing="0"  width="" id="templateContainer">
								<tr>
									<td align="center" valign="top">
										...
										<table border="0" cellpadding="0" cellspacing="0" width="100%" id="templateHeader">
											<tr>
												<td class="headerContent">
													<map name="foto">
														<area shape="rect" coords="70,395,255,435" href="http://www.salcobrand.cl/" target="_blank">
														<area shape="rect" coords="70,440,255,475" href="http://www.salcobrand.cl/" target="_blank">
														<area shape="rect" coords="290,395,530,435" href="http://www.facebook.com/salcobrand" target="_blank">
														<area shape="rect" coords="290,440,530,480" href="http://www.twitter.com/salcobrand" target="_blank">
													</map>
													<!-- // Begin Module: Standard Header Image \\ -->
													<img align="left" src="http://www.salcobrand.cl/cl/campana_registro/imagenes/img2.jpg" usemap="#foto">
...
if(!function_exists('salco_asociado_region')){
	function salco_asociado_region(){
		global $wpdb;
		$key = $wpdb->get_results($wpdb->prepare("SELECT region FROM ".$wpdb->prefix."asociados GROUP BY region ORDER BY idregion ASC"));
		if(count($key)){
			return $key;
		}else{
			return Array();
...
# Agregado por Cristobal el 13/08/2012
...
if(!function_exists('salco_referidos')){
	function salco_referidos(){
									...
									<td align="center" valign="top">
										...
										<table border="0" cellpadding="0" cellspacing="0" width="100%" id="templateHeader">
											<tr>
												<td class="headerContent">
													<map name="foto">
														<area shape="rect" coords="70,555,255,590" href="http://www.salcobrand.cl/" target="_blank">
														<area shape="rect" coords="240,595,67,630" href="http://www.salcobrand.cl/" target="_blank">
														<area shape="rect" coords="270,335,515,405" href="http://www.salcobrand.cl" target="_blank">
														<area shape="rect" coords="290,550,530,595" href="http://www.facebook.com/salcobrand" target="_blank">
														<area shape="rect" coords="290,600,530,630" href="http://www.twitter.com/salcobrand" target="_blank">
													</map>
													<!-- // Begin Module: Standard Header Image \\ -->
													<img align="left" src="http://www.salcobrand.cl/cl/campana_registro/imagenes/img1.jpg" usemap="#foto">
													<!-- // End Module: Standard Header Image \\ -->
	...
	wp_mail($_POST['email_amigo1'], '¡Inscríbete y participa por una de las 3 plataformas vibratorias!', $message, $headers);
	wp_mail($_POST['email_amigo2'], '¡Inscríbete y participa por una de las 3 plataformas vibratorias!', $message, $headers);
	wp_mail($_POST['email_amigo3'], '¡Inscríbete y participa por una de las 3 plataformas vibratorias!', $message, $headers);		
...
### CAMPAÑA REGISTRO 2013 ###

function salco_consultacli(){
	...
	$query = "SELECT COUNT(id) as total FROM sb_users WHERE user_nicename LIKE '%".$rut."%'";

	$usuario = $wpdb->get_var($query);
...
###########################################################################
### FUNCIONES PARA CONSULTA PRECIO, CREADAS POR TWITTOBAL.NET MAYO 2014 ###
###########################################################################


function salco_buscarproducto(){
	global $wpdb;
	$cms_oracle = new cms_oracle();
	$resultados = $cms_oracle->buscarProducto($_POST['producto'],'1000',$_POST['rut'],$_POST['perfil']);
	echo json_encode($resultados);


	$wpdb->query($wpdb->prepare('INSERT INTO sb_busqueda_productos(busqueda, rut, ip, fecha) VALUES ("'.$_POST['producto'].'","'.$_POST['rut'].'","'.$_SERVER['REMOTE_ADDR'].'",now());'));
...
function salco_obtieneperfil(){

	$cms_oracle = new cms_oracle();
	$perfil = $cms_oracle->obtienePerfil($_POST['rut']);
	$cantidad_perfiles = ceil(strlen($perfil) / 80);
	$pos_inicio = 65;
	$pos_inicio2 = 70;

	for($x=0;$x<$cantidad_perfiles;$x++){
		$perfiles[] = 	array(
							'cod_perfil'=>trim(substr($perfil,$pos_inicio,5)),
							'nombre_perfil'=>strtoupper(trim(substr($perfil,$pos_inicio2,25)))
						);
		$pos_inicio += 46;
		$pos_inicio2 += 46;
	}
...
function salco_obtienepreciopromocion(){
	
	$cms_oracle = new cms_oracle();
	$resultados = $cms_oracle->buscarProducto($_POST['producto'],$_POST['local']);
	print_r($resultados);
	die();
}
