...
if(!function_exists('salco_contacto')){
	function salco_contacto(){
		...
		if ($asunto == "Exclusivo Tarjeta Crédito"){
			...
			wp_mail("danielafresard@gmail.com", "Consulta", "hola", $headers);
			wp_mail("javieres1985@gmail.com", "Consulta", "hola", $headers);
...
if(!function_exists('salco_asociado')){
	function salco_asociado(){
		...
		if($comuna_id == "" || $rubro_id == ""){
			$query = "SELECT * FROM ".$wpdb->prefix."asociados ORDER BY RAND() LIMIT 0";
		}else{
...
if(!function_exists('salco_asociado_region')){
	function salco_asociado_region(){
		global $wpdb;
		$key = $wpdb->get_results($wpdb->prepare("SELECT region FROM ".$wpdb->prefix."asociados GROUP BY region ORDER BY region DESC"));
		if(count($key)){
			return $key;
		}else{
			return Array();
...
if(!function_exists('recoge_puntos')){
	...
	/*
		$fullrut = $this->obj->cms_session->get_vars("fullrut");
		$rut = explode("-", $fullrut);
		return $this->obj->cms_oracle->getPuntos(Array('rut' => $rut[0]));
	*/
		$cms_oracle = new cms_oracle();
		$rut[0] 	= "17069852";
		//echo ($rut[0]);
